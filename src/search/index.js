...
const SearchComponent = ()=>{
    ...
    return(
        <div>
            ...
            <div className="row">
                {!loading && data.results.map(result=><SearchResult key={result.id} result={result}/>)}
            </div>
            {!loading &&
             <div className="row">
                 <div className="col">
                     {data.previous &&
                      <button className="btn btn-primary rounded float-start p-3 w-25"
                              onClick={()=>{prevNextClick(data.previous)}}>
                          Prev
                      </button>}
                     {data.next && <button className="btn btn-primary rounded float-end p-3 w-25"
                                           onClick={()=>{prevNextClick(data.next)}}>Next</button>}
                 </div>
