...
const EditProfile=()=>{
  ...
  const updateProfileHandler= async () => {
    ...
    console.log("calling thunk",profile)
    if(response)
    {console.log("bef dispatch",{...profile,...response})
  ...
  const upload=async ()=>{
    ...
    { const imgcoverName=profile.username+"coverpic.jpg"
      const response = await axios.get(`${BACKEND_API}/image/s3Url/${imgcoverName}`);
      const {url} = response.data;
      console.log("url" , url)
      const response1= await fetch(url, {
        method: "PUT",
      ...
      if(response1) {
        ...
        console.log("cover imageUrl1 ", coverimageUrl)
        setProfile(prevState => ({...prevState, cover_pic: "dhan"}))
        console.log("cover imageUrl2", coverimageUrl)
  ...
  return(
      <div className="border border-secondary rounded-4 ">
        ...
        <div className="row">
          <div className="col">
            <img alt="Banner"
                 src={profile.cover_pic} className="w-100" height={240}/>
          </div>
        </div>
        ...
        <div className="pt-3">
          <form>
            ...
            <div className="form-group row p-2">
              ...
              <div className="col-sm-10">
                <input className="form-control" type="date"
                       onChange={(e)=>{ setProfile(prevState=>({...prevState,dob:e.target.value.toString()}))}}
                       id="dateofbirth" value={profile.dob}/>
              </div>
            </div>
            <div className="form-group row p-2">
              <label htmlFor="profilepic"
                     className="col-sm-2 col-form-label">Profile pic</label>
              <div className="col-sm-10">
                <input className="form-control" type="file"
                       id="profilepic" onChange={updateProfilePic}/>
              </div>
            </div>
            <div className="form-group row p-2">
              <label htmlFor="coverpic"
                     className="col-sm-2 col-form-label">Cover pic</label>
              <div className="col-sm-10">
                <input className="form-control" type="file"
                       id="coverpic" onChange={updateCoverPic}/>
              </div>
            </div>
            <div className="form-group row p-2">
            ...
            <div className="form-group row p-2">
              ...
              <div className="col-sm-10">
                <input type="text" className="form-control"
                       onChange={(e)=>{setProfile(prevState=>({...prevState,phone:e.target.value}))}}
            ...
            <div className="form-group row p-2">
              ...
              <div className="col-sm-10">
                <input type="text" className="form-control"
                       onChange={(e)=>{setProfile(prevState=>({...prevState,email:e.target.value}))}}
