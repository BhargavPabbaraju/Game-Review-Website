...
const DetailComponent = () => {
    ...
    let bg_image ="https://www.thermaxglobal.com/wp-content/uploads/2020/05/image-not-found.jpg";
    return (
        <div>
             ...
             <>
                 ...
                 <div className="row mb-3">
                     {game.background_image ?
                      <img alt="game bg" src={game.background_image} className="w-100 rounded-5"
                           height={350}/> :
                      <img alt="game bg" src={bg_image} className="w-100 rounded-5"
                           height={350}/>
                     }
                 </div>
                 <div className="row">
                     {game.genres.map(genre=><GenreItem key={genre.id} genre={genre.name}/>)}
                     {game.tags.map(genre=><GenreItem key={genre.id} genre={genre.name}/>)}
                 </div>
                 <div className="row">
                 ...
                 <div className="row mb-3">
                     {game.stores.map(store=><div className="col-auto text-primary">
                         <a href={`https:\\${store.store.domain}`} target="_blank" rel="noopener noreferrer">
                             {store.store.name}
                         </a>
